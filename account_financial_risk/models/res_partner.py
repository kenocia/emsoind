...
class ResPartner(models.Model):
    ...
    financial_risk_enabled = fields.Boolean(
        compute="_compute_financial_risk_enabled",
        string="Financial Risk Applies",
        help="Financial risk control is only enforced for company parent "
        "contacts whose payment terms are different from immediate payment.",
    )
    ...
    @api.depends("is_company", "parent_id", "property_payment_term_id")
    def _compute_financial_risk_enabled(self):
        immediate_term = self._get_immediate_payment_term()
        for partner in self:
            term = partner.property_payment_term_id
            is_credit_term = bool(term) and (
                not immediate_term or term.id != immediate_term.id
            )
            partner.financial_risk_enabled = bool(
                partner.is_company and not partner.parent_id and is_credit_term
            )

    def _get_risk_exceeded_html(self, extra_amount, document_label):
        """Build the rich credit-limit-exceeded message shown to the user."""
        self.ensure_one()
        partner = self.commercial_partner_id
        currency = partner.risk_currency_id
        credit_limit = partner.sudo().credit_limit
        risk_total = partner.risk_total
        available = credit_limit - risk_total
        new_total = risk_total + extra_amount
        exceeded = new_total - credit_limit

        def _fmt(amount):
            return formatLang(self.env, amount, currency_obj=currency)

        rows = [
            (self.env._("Credit limit"), _fmt(credit_limit)),
            (self.env._("Current risk"), _fmt(risk_total)),
            (self.env._("Available credit"), _fmt(available)),
            (
                self.env._("Amount of this %s", document_label),
                _fmt(extra_amount),
            ),
            (self.env._("New total risk"), _fmt(new_total)),
            (self.env._("Would exceed by"), _fmt(exceeded)),
        ]
        items = "".join(
            "<li>{label}: <strong>{value}</strong></li>".format(label=label, value=value)
        ...
        return Markup(
            ...
            title=self.env._("FINANCIAL RISK DETECTED"),
            intro=self.env._(
                "This %(doc)s would exceed the credit limit of customer %(name)s.",
                doc=document_label,
    ...
    def _compute_risk_exception(self):
        ...
        for partner in self:
            ...
            # Financial risk is only enforced for company parent contacts with
            # credit payment terms (different from immediate payment).
    ...
    def _get_depends_compute_risk_exception(self):
        ...
        res.append("financial_risk_enabled")
        return res
