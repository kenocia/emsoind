...
export class AutoCompleteBits extends Component {
    static template = "awdw.AutoCompleteBits";
    static props = {
    ...
    async loadSources(useInput) {
        ...
        let avail_models = ['res.users','res.company']
        if(resolved && this.sources.length > 0 && avail_models.includes(this.props.resmodel)) {
            let index_arr = this.sources[0].options.map((e)=>e.id);
            let index_size = [index_arr.length -1]
            if(this.props.resmodel == 'res.users'){
                this.sources[0].options.push({
                    'id':index_size+1,'label':'Environment User','value':0
                })
            }
            if(this.props.resmodel == 'res.company'){
                this.sources[0].options.push({
                    'id':index_size+1,'label':'Environment Company','value':0
                })
            }
